...
include_once('backend/conect.php');
    ...
    <div class="container">
      ...
      <form class="form-login" method="post">
        ...
        <div class="login-wrap">
          <input type="text" class="form-control" name="email_cost" placeholder="Digite seu email..." autofocus>
          <br>
          <input type="password" name="senha_cost" class="form-control" placeholder="Digite sua senha...">
      ...
      <?php
							...
							if (isset($_GET['acao'])) {
								...
								if (isset($_POST['login'])){
				    				$login = filter_input(INPUT_POST, 'email_cost', FILTER_DEFAULT);
						        $senha = base64_encode(filter_input(INPUT_POST, 'senha_cost', FILTER_DEFAULT));
						        $status = filter_input(INPUT_POST, 'status', FILTER_DEFAULT);
						        $select = "SELECT * FROM tb_costureira WHERE email_cost=:emailLogin AND senha_cost=:senhaLogin AND status=:status";
						        ...
						        try {
						          ...
						          if ($verificar = 1) {
						          	$login = $_POST['email_cost'];
						            $senha = trim(strip_tags(base64_encode($_POST['senha_cost'])));
